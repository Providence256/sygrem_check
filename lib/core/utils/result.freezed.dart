...
class _$FailureImpl<T> implements Failure<T> {
  const _$FailureImpl(this.message, {this.code});
  ...
  @override
  String toString() {
    return 'Result<$T>.failure(message: $message, code: $code)';
  }
...
abstract class Failure<T> implements Result<T> {
  const factory Failure(final String message, {final String? code}) =
      _$FailureImpl<T>;
